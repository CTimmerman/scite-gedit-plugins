...
class window_helper:
    ...
    def update_ui(self):
        ...
        # Switch bookmark store for current document
    ...
    def _insert_menu(self):
        ...
        self._next_bookmark.connect("activate", self._on_next_bookmark)

        self._previous_bookmark = gtk.Action("PreviousBookmark", _("Previous Bookmark"), _("Previous Bookmark"), None)
        self._previous_bookmark.connect("activate", self._on_previous_bookmark)
        ...
        while hot_key < 10:
            ...
            self._act_hot_key[hot_key].connect("activate", self._on_toggle_bookmark, False, hot_key)
            hot_key += 1

        self._action_group.add_action(self._act_ab)
        self._action_group.add_action(self._act_nb)
        self._action_group.add_action_with_accel(self._act_toggle, "<Ctrl>F2")
    ...
    def _remove_menu(self):
        ...
        # Make sur?//?e the manager updates        
    ...
    def _on_next_bookmark(self, action):
        ...
        if doc:
            uri = doc.get_uri()
            if self._bookmarks._list.has_key(uri):
                bookmark_lines = sorted(self._bookmarks._list[uri]["iters"].keys())
                if bookmark_lines:
                    text_iter = doc.get_iter_at_mark(doc.get_insert())
                    current_line = text_iter.get_line()+1
                    for line in bookmark_lines:
                        if line > current_line:
                            break
                    else:
                        # wrap around
                        line = bookmark_lines[0]
