...
class bookmark_list(object):
    ...
    def add(self, uri, line, source, comment="", hot_key=None): # Adds a line for an uri (returns True on success)
...
#            content = source
    ...
    def toggle(self, uri, line, source, comment="", hot_key=None): # Adds or removes a line for an uri
        if self.exists(uri, line):
            self.delete(uri, line)
            return False
        else:
            self.add(uri, line, source, comment, hot_key)
            return True
